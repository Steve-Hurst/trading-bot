...
echo Generating AutoTraderBotIG\buildinfo.cs (SHA: !GIT_SHA!, Branch: !GIT_BRANCH!, Control Token: !COOKIE_CONTROL_TOKEN!)...
...
echo         public const string AlgorithmName = "EURUSD_MeanReversion_IG_SpreadBet";
...
echo         public const int DefaultPort = 9012;
echo         public const string ServiceName = "%APP_NAME%";
echo         public const string ServiceDescription = "Steve Hurst AutoTrader IG Bot 01 - Spread Betting Execution Engine (IG Index UK)";
echo     }
echo }
) > "%PROJ_DIR%AutoTraderBotIG\buildinfo.cs"

echo %DATE% %TIME% Building %APP_NAME%...
echo %DATE% %TIME% Building %APP_NAME%...>>"%LOG_DIR%\%LOG_FILE%"

dotnet publish AutoTraderBotIG\AutoTraderBotIG.csproj -c Release -r win-x64 -p:PublishSingleFile=true --self-contained false -o "%BIN_DIR%" >>"%LOG_DIR%\%LOG_FILE%" 2>&1
if errorlevel 1 (
